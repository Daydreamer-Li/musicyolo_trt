...
def get_notes(filepath):
    notes = []
    #filepath = filepath.replace('.flac.pred','')
...
if __name__=='__main__':
    ...
    for filename in filenames:
        video_flag = filename.split('.')[0]
        labelpath = os.path.join(label_path, video_flag+'.txt')
        ...
        print()
        print(filename)
        print('op F1: %4.2f%% P: %4.2f%% R:  %4.2f%%'%(f*100, p*100, r*100))
        ...
        oop[2].append(r)
        if offset:
            print('noop F1: %4.2f%% P: %4.2f%% R:  %4.2f%%'%(f*100, p*100, r*100))
    ...
    if offset:
        print('\nmean oop F1: %4.2f%% P: %4.2f%% R:  %4.2f%%'%tuple(mean_oop))